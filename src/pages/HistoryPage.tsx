...
export function HistoryPage({
  ...
  const allProviders = useMemo(() => {
    const providers = Array.from(new Set(mockHistory.map(r => (r.provider || '').trim()).filter(Boolean)));
  ...
  const filteredMock = useMemo(() => {
    if (providerFilter === 'all') return mockHistory;
    return mockHistory.filter(r => r.provider === providerFilter);
  }, [providerFilter, mockHistory]);
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1">
                {tab === 'official'
                  ? `총 ${history.length}개의 채점 기록`
                  : `총 ${mockHistory.length}개의 사설 기록`}
              </p>
            </div>
        ...
        {tab === 'official' ? (
          history.length === 0 ? (
            <div className="bg-white rounded-lg shadow p-8 text-center">
              <p className="text-gray-600 mb-4">아직 채점한 기출 기록이 없습니다.</p>
              <button
                onClick={() => navigate('/')}
