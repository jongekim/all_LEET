...
`main` push는 배포 시작 신호이며 배포 완료가 아니다. Vercel 빌드·배포·운영 도메인 전환에는 수십 초에서 몇 분 걸릴 수 있다. push한 **커밋의 Vercel 상태**를 먼저 확인하고, 진행 중이면 15~30초 간격으로 다시 확인한다. `success`가 된 뒤 운영 URL의 상태 코드와 대표 HTML을 점검한다. 성공 직후 운영 도메인이 잠시 이전 응답을 주면 잠깐 기다려 재확인한다. 명시적인 `failure`는 기다려도 해결되는 지연으로 취급하지 않고 배포 로그와 설정을 확인한다.
