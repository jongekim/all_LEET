...
공개 페이지의 사이트맵은 `src/public/sitemap.xml`에 수동으로 관리한다. 정적 경로의 검색 메타데이터는 `App.tsx`가 설정하고, `/past-exams`는 선택한 학년도·과목·문형에 맞는 제목·설명과 정규화된 쿼리 canonical을 설정한다. `/community/:id`는 기존 게시글 조회 결과로 제목과 설명을 갱신한다. 모든 경로는 동일한 `index.html`을 받으므로 경로별 메타데이터는 브라우저에서 React가 실행된 뒤 적용된다.
