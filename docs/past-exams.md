...
정답표와 점수 환산표는 PDF 영역 아래의 `정답표·점수 환산표` 카드를 직접 눌렀을 때만 함께 표시한다. 카드에는 `문제를 푼 뒤 눌러서 확인하세요.` 안내와 보기 문구·화살표를 표시하며, 카드 전체를 클릭하거나 키보드로 조작할 수 있다. 기본 상태는 접힘이며, 학년도·과목·시험 유형 변경, 뒤로가기/앞으로가기로 시험 선택 변경, 새로고침 후에도 다시 접힌다. 펼침 상태는 URL이나 브라우저 저장소에 저장하지 않는다.
