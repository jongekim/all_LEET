...
create policy if not exists "chat_profiles_select_all"
on public.chat_profiles
for select
using (true);

-- Only the owner can create their profile
create policy if not exists "chat_profiles_insert_own"
on public.chat_profiles
for insert
with check (auth.uid() = user_id);

-- Only the owner can update their own profile
create policy if not exists "chat_profiles_update_own"
on public.chat_profiles
for update
...
create policy if not exists "chat_messages_insert_auth"
on public.chat_messages
for insert
with check (auth.uid() = user_id);
